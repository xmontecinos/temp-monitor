...
# Función para dar color a la tabla sin requerir matplotlib
def color_temperatura(val):
    if val >= UMBRAL_CRITICO: color = '#fee2e2' # Rojo claro
    elif val >= UMBRAL_PREVENTIVO: color = '#fef9c3' # Amarillo claro
    else: color = '#dcfce7' # Verde claro
...
if archivos_lista:
    ...
    # --- PESTAÑA 2: BUSCADOR (CORREGIDA) ---
    with tab_busq:
        sitio_busq = st.selectbox("Seleccionar Sitio:", sorted(df_actual['Sitio'].unique()))
        df_res = df_actual[df_actual['Sitio'] == sitio_busq]
        # Usamos applymap para dar color sin necesitar matplotlib
        st.dataframe(df_res.style.applymap(color_temperatura, subset=['Temp']), use_container_width=True)
        ...
        with c1:
            ...
            if st.button("🔥 Generar/Actualizar Parquet"):
                ...
                if all_dfs:
                    ...
                    p_text.success(f"✅ ¡Parquet creado!")
