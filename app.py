...
if archivos_lista:
    ...
    with tab_hist:
        ...
        num_reportes = st.slider("Seleccione cantidad de archivos a procesar:", 5, min(300, len(archivos_lista)), 50)
        
        if st.button("🚀 Iniciar Procesamiento Histórico"):
            all_data = []
            # --- SISTEMA DE PROGRESO VISUAL ---
            progreso_texto = st.empty()  # Espacio para el nombre del archivo
            barra_progreso = st.progress(0)
            
            for i, path in enumerate(archivos_lista[:num_reportes]):
                nombre_archivo = os.path.basename(path)
                # Actualizar texto y barra
                progreso_texto.text(f"Procesando ({i+1}/{num_reportes}): {nombre_archivo}")
                barra_progreso.progress((i + 1) / num_reportes)
                
                all_data.extend(extraer_datos_masivo(path))
            
            if all_data:
                df_h = pd.DataFrame(all_data)
                # Agrupar para optimizar memoria
                st.session_state["df_full"] = df_h.groupby([df_h['Timestamp'].dt.floor('h'), 'Sitio', 'ID_Full'])['Temp'].max().reset_index()
                progreso_texto.success(f"✅ ¡Carga completada! {num_reportes} archivos procesados.")
                barra_progreso.empty() # Quita la barra al terminar
            else:
                st.error("No se pudo extraer información de los archivos.")

        # Mostrar gráficos si los datos ya existen en memoria
        if "df_full" in st.session_state:
            st.divider()
            df_p = st.session_state["df_full"]
            sitio_sel = st.selectbox("Seleccione Sitio para ver tendencia:", sorted(df_p['Sitio'].unique()))
            df_s = df_p[df_p['Sitio'] == sitio_sel]
            ids = sorted(df_s['ID_Full'].unique())
            sel = st.multiselect("Filtrar por Slot:", ids, default=ids)
            if sel:
                fig_l = px.line(df_s[df_s['ID_Full'].isin(sel)], x='Timestamp', y='Temp', color='ID_Full', title=f"Evolución Térmica - {sitio_sel}")
