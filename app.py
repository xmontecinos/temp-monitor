...
if archivos_lista:
    ...
    with tab_hist:
        ...
        if "df_full" in st.session_state:
            ...
            c1, c2 = st.columns(2)
            if c1.button("✅ Todos"): st.session_state["sel"] = ids
            if c2.button("❌ Ninguno"): st.session_state["sel"] = []
            
            sel = st.multiselect("ID:", ids, default=st.session_state.get("sel", ids))
            if sel:
                fig_l = px.line(df_s[df_s['ID_Full'].isin(sel)], x='Timestamp', y='Temp', color='ID_Full')
                st.plotly_chart(fig_l, use_container_width=True)
else:
    st.error("No se detectaron archivos en 'Temperatura'.")
